...
class ScamDetector:
    ...
    URGENCY_PATTERNS = [
        ...
        r'\basap\b', r'\bquick\b', r'\bfast\b', r'\bhurry\b',
        r'\blast\s+chance\b', r'\blimited\s+time\b', r'\bexpir(e|ing|ed)\b',
    ]
    
    THREAT_PATTERNS = [
        r'\bblock(ed)?\b', r'\bsuspend(ed)?\b', r'\bdeactivat(e|ed)\b',
        r'\bfreez(e|ing)\b', r'\bclose[ds]?\b', r'\blegal\s+action\b',
        r'\barrest\b', r'\bpolice\b', r'\bcourt\b', r'\bpenalty\b',
        r'\bfine\b', r'\bwarrant\b',
    ]
    
    REQUEST_PATTERNS = [
        r'\bverify\b', r'\bconfirm\b', r'\bupdate\b', r'\bprovide\b',
        r'\bshare\b', r'\bsend\b', r'\btransfer\b', r'\bpay\b',
        r'\bclick\b', r'\blink\b', r'\benter\b',
    ]
    
    SENSITIVE_DATA_PATTERNS = [
        r'\botp\b', r'\bpin\b', r'\bpassword\b', r'\bcvv\b',
        r'\bcard\s+number\b', r'\baccount\s+number\b', r'\bbank\s+details\b',
        r'\bupi\b', r'\baadhaar\b', r'\bpan\b', r'\bkyc\b',
    ]
    
    IMPERSONATION_PATTERNS = [
        r'\bbank\b', r'\brbi\b', r'\bsbi\b', r'\bhdfc\b', r'\bicici\b',
        r'\baxis\b', r'\bpaytm\b', r'\bgpay\b', r'\bphonepe\b',
        r'\bamazon\b', r'\bflipkart\b', r'\bcustomer\s+(care|service)\b',
        r'\bgovernment\b', r'\bofficial\b', r'\bauthorized\b',
    ]
    
    MONEY_PATTERNS = [
        r'₹\s*\d+', r'\brs\.?\s*\d+', r'\brupees?\b', r'\binr\b',
        r'\blakh\b', r'\bcrore\b', r'\bprize\b', r'\blottery\b',
        r'\bwinner\b', r'\bcashback\b', r'\breward\b', r'\bbonus\b',
    ]
    ...
    def analyze_message(self, message: Message) -> Tuple[float, List[str]]:
        ...
        if urgency_count > 0:
            score += min(urgency_count * 0.05, 0.15)
        ...
        if threat_count > 0:
            score += min(threat_count * 0.08, 0.25)
        ...
        if request_count > 0:
            score += min(request_count * 0.05, 0.15)
        ...
        if sensitive_count > 0:
            score += min(sensitive_count * 0.08, 0.25)
        ...
        if impersonation_count > 0:
            score += min(impersonation_count * 0.05, 0.10)
        ...
        if money_count > 0:
            score += min(money_count * 0.05, 0.10)
            keywords.extend(self._extract_matched_keywords(text, self.money_re))
        
        return min(score, 1.0), list(set(keywords))
    ...
    def analyze_session(self, session: SessionState) -> Tuple[float, bool, bool, List[str]]:
        ...
        # Determine status
        is_suspected = final_score >= 0.3
        is_confirmed = final_score >= 0.6 and message_count >= 2
